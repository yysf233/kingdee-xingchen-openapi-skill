---
id: "kdcloud-9f42d6ccc38c11efb6442787d01017a4"
title: "销售汇总表列表"
source: "金蝶云社区"
url: "https://open.jdy.com/#/files/api/detail?index=3&categrayId=&id=9f42d6ccc38c11efb6442787d01017a4"
author: ""
published_at: ""
product: "金蝶云苍穹"
module: ["进销存云", "销售单据", "销售汇总表"]
language: "Kingscript"
version_hint: ["V2"]
keywords: []
quality: "B"
---

# 销售汇总表列表

### 基本信息

-   用途说明：销售汇总表列表
-   请求方式：GET
-   请求地址：https://api.kingdee.com/jdy/v2/sal/sal\_summary\_rpt\_ims

### 请求参数

#### headers参数

| 参数名称 | 参数示例 | 是否必填 | 参数说明 |
| --- | --- | --- | --- |
| Content-Type | application/json | true | 固定传：application/json |
| X-Api-ClientID | 205022 | true | 应用id，[创建指引](https://open.jdy.com/#/files/api/detail?index=3&categrayId=316e1f5bd9d711ed8e36c17691e84ff5&id=a540e3dcd9d811ed8e3677dc79b66e86&noside=true) |
| X-Api-Auth-Version | 2.0 | true | 固定传:2.0 |
| X-Api-TimeStamp | 1655775240000 | true | 当前时间毫秒时间戳，有效期为5min |
| X-Api-SignHeaders | X-Api-TimeStamp,X-Api-Nonce | true | 固定传：X-Api-TimeStamp,X-Api-Nonce |
| X-Api-Nonce | 1655775240000 | true | 随机正整数 |
| X-Api-Signature | xxx | true | [参考加密规则](https://open.jdy.com/#/files/api/detail?index=3&categrayId=5403e0fd6a5811eda819b759130d6d33&id=b5c26557da9d11ed8e36f9799578d2e1&noside=true) |
| app-token | xxx | true | 产品账套级别的token，[获取指引](https://open.jdy.com/#/files/api/detail?index=3&categrayId=5403e0fd6a5811eda819b759130d6d33&id=9076d1a6da9d11ed8e36874bed64e0be&noside=true) |
| X-GW-Router-Addr | https://tf.jdy.com | true | IDC域名，获取appKey返回消息体内的domain字段 |

#### params参数

| 参数名称 | 参数类型 | 是否必填 | 参数说明 |
| --- | --- | --- | --- |
| filter\_summary\_field | string | true | 汇总依据,customerid：按客户 |
| end\_date | string | false | 结束日期（“yyyy-MM-dd”）不传默认查本月，需要与start\_date一起传递，开始日期和结束日期需要限定在6个月内，超过会报错 |
| filter\_material\_id | array | false | 商品id，格式：\[“id”,“id”,“xxx”\] |
| page | integer | false | 当前页，默认1 |
| page\_size | integer | false | 每页条数,默认100 |
| show\_package | boolean | false | 是否查询整件散包，true-是，false-否,不传默认false |
| start\_date | string | false | 开始日期（“yyyy-MM-dd”）不传默认查本月，需要与end\_date一起传递 |

#### 请求示例

```bash
curl --location --request  get 'https://api.kingdee.com/jdy/v2/sal/sal_summary_rpt_ims?filter_summary_field=customerid&end_date=2022-01-01&filter_material_id=[1,2,3]&page=1&page_size=100&show_package=false&start_date=2022-01-01' \
--header 'Content-Type: application/json' \
--header 'X-Api-ClientID: 205022' \
--header 'X-Api-Auth-Version: 2.0' \
--header 'X-Api-TimeStamp: 1655775240000' \
--header 'X-Api-Nonce: 1655775240000' \
--header 'X-Api-SignHeaders: X-Api-TimeStamp,X-Api-Nonce' \
--header 'X-Api-Signature: xxx' \
--header 'app-token: xxx' \
--header 'X-GW-Router-Addr: https://tf.jdy.com'
```

### 响应参数

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| errcode | integer | 返回码，成功时为0 |
| description | string | 返回信息，成功时为success，失败时为具体信息 |
| data | Data | 业务返回具体对象 |

**Data**

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| count | string | 总条数 |
| cur\_page\_size | integer | 每页条数,默认100 |
| page | integer | 当前页，默认1 |
| page\_size | integer | 每页条数 |
| rows | List <Row> | 数据行 |
| total\_page | integer | 总页数 |

**Row**

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| act\_non\_tax\_amount | string | 实际不含税金额,精度是小数点后10位 |
| all\_amount | string | 价税合计,精度是小数点后10位 |
| base\_qty | number | 销售数量 |
| base\_unit\_id | string | 基本单位id |
| base\_unit\_name | string | 基本单位名称 |
| customer\_id | string | 客户id |
| customer\_name | string | 客户名称 |
| material\_category | string | 商品类别 |
| material\_id | string | 商品id |
| material\_name | string | 商品名称 |
| material\_number | string | 商品编码 |
| package | string | 整件散包 |
| tax\_amount | string | 税额,精度是小数点后10位 |

#### 响应示例

```json
{
  "data": {
    "count": "1",
    "cur_page_size": 100,
    "page": 1,
    "page_size": 100,
    "rows": [
      {
        "act_non_tax_amount": "1",
        "all_amount": "100",
        "base_qty": 22,
        "base_unit_id": "1",
        "base_unit_name": "1a4",
        "customer_id": "1",
        "customer_name": "客户名称",
        "material_category": "电子产品",
        "material_id": "1",
        "material_name": "耳机",
        "material_number": "SP00015",
        "package": "10cm",
        "tax_amount": "1"
      }
    ],
    "total_page": 100
  },
  "description": "success",
  "errcode": 0
}
```

#### 返回码

| 返回码 | 描述 | 解决方案 |
| --- | --- | --- |
| [公告返回码](https://open.jdy.com/#/files/api/detail?index=2&categrayId=1f51c576013945e2af68ef15d4245a48&id=525e704824d24b178ab466530456c037) | 公告返回码 |  |
| 2000002000 | 系统错误,请到开发者社区提单反馈 | [开发者社区](https://vip.kingdee.com/developer?productLineId=29) |
