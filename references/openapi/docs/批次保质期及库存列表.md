---
id: "kdcloud-9e25330b712511eda0b31de233aa3797"
title: "批次保质期及库存列表"
source: "金蝶云社区"
url: "https://open.jdy.com/#/files/api/detail?index=3&categrayId=&id=9e25330b712511eda0b31de233aa3797"
author: ""
published_at: ""
product: "金蝶云苍穹"
module: ["进销存云", "仓存报表"]
language: "Unknown"
version_hint: ["V2"]
keywords: []
quality: "B"
---

# 批次保质期及库存列表

### 基本信息

-   用途说明：批次保质期及库存列表
-   请求方式：GET
-   请求地址：https://api.kingdee.com/jdy/v2/scm/batch\_kfperiod\_inventory

### 请求参数

#### headers参数

| 参数名称 | 参数示例 | 是否必填 | 参数说明 |
| --- | --- | --- | --- |
| Content-Type | application/json | true | 固定传：application/json |
| X-Api-ClientID | 205022 | true | 应用id，[创建指引](https://open.jdy.com/#/files/api/detail?index=3&categrayId=316e1f5bd9d711ed8e36c17691e84ff5&id=a540e3dcd9d811ed8e3677dc79b66e86&noside=true) |
| X-Api-Auth-Version | 2.0 | true | 固定传:2.0 |
| X-Api-TimeStamp | 1655775240000 | true | 当前时间毫秒时间戳，有效期为5min |
| X-Api-SignHeaders | X-Api-TimeStamp,X-Api-Nonce | true | 固定传：X-Api-TimeStamp,X-Api-Nonce |
| X-Api-Nonce | 1655775240000 | true | 随机正整数 |
| X-Api-Signature | xxx | true | [参考加密规则](https://open.jdy.com/#/files/api/detail?index=3&categrayId=5403e0fd6a5811eda819b759130d6d33&id=b5c26557da9d11ed8e36f9799578d2e1&noside=true) |
| app-token | xxx | true | 产品账套级别的token，[获取指引](https://open.jdy.com/#/files/api/detail?index=3&categrayId=5403e0fd6a5811eda819b759130d6d33&id=9076d1a6da9d11ed8e36874bed64e0be&noside=true) |
| X-GW-Router-Addr | https://tf.jdy.com | true | IDC域名，获取appKey返回消息体内的domain字段 |

#### params参数

| 参数名称 | 参数类型 | 是否必填 | 参数说明 |
| --- | --- | --- | --- |
| aux\_prop\_ids | string | false | 商品辅助属性ID集合，多个ID用英文逗号隔开 |
| create\_end\_time | string | false | 创建时间-结束时间的时间戳(毫秒) |
| create\_start\_time | string | false | 创建时间-开始时间的时间戳(毫秒) |
| material\_ids | string | false | 商品ID集合，多个ID用英文逗号隔开 |
| modify\_end\_time | string | false | 修改时间-结束时间的时间戳(毫秒) |
| modify\_start\_time | string | false | 修改时间-开始时间的时间戳(毫秒) |
| page | string | false | 当前页，默认1 |
| page\_size | string | false | 每页显示条数默认10 |
| sp\_ids | string | false | 仓位ID集合，多个ID用英文逗号隔开 |
| stock\_ids | string | false | 仓库ID集合，多个ID用英文逗号隔开 |

#### 请求示例

```bash
curl --location --request  get 'https://api.kingdee.com/jdy/v2/scm/batch_kfperiod_inventory?aux_prop_ids=11,22,33&create_end_time=1234567890123&create_start_time=1234567890123&material_ids=11,22,33&modify_end_time=1234567890123&modify_start_time=1234567890123&page=1&page_size=100&sp_ids=11,22,33&stock_ids=11,22,33' \
--header 'Content-Type: application/json' \
--header 'X-Api-ClientID: 205022' \
--header 'X-Api-Auth-Version: 2.0' \
--header 'X-Api-TimeStamp: 1655775240000' \
--header 'X-Api-Nonce: 1655775240000' \
--header 'X-Api-SignHeaders: X-Api-TimeStamp,X-Api-Nonce' \
--header 'X-Api-Signature: xxx' \
--header 'app-token: xxx' \
--header 'X-GW-Router-Addr: https://tf.jdy.com'
```

### 响应参数

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| errcode | integer | 返回码，成功时为0 |
| description | string | 返回信息，成功时为success，失败时为具体信息 |
| data | Data | 业务返回具体对象 |

**Data**

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| count | integer | 总计条目 |
| current\_page\_size | integer | 当前页大小 |
| page | integer | 当前页 |
| page\_size | integer | 页大小 |
| rows | List <Row> | 返回数据列表 |
| total\_page | integer | 总页数 |

**Row**

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| aux\_prop\_entry | AuxPropEntry | 辅助属性 |
| aux\_qty | string | 辅助基本单位库存数量 |
| batch\_no | string | 批次号 |
| id | string | ID |
| kf\_date | string | 生产日期 |
| kf\_period | integer | 保质期时长 |
| kf\_type | string | 保质期单位，1：天，2：月，3：年 |
| material\_entry | MaterIalEntry | 商品 |
| qty | string | 即时库存数量（基本单位） |
| sp\_entry | SpEntry | 仓位 |
| stock\_entry | StockEntry | 仓库 |
| valid\_aux\_qty | string | 辅助基本单位库存可用量 |
| valid\_date | string | 到期日 |
| valid\_qty | string | 即时库存可用量 |

**StockEntry**

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| id | string | 仓库id |
| name | string | 仓库名称 |
| number | string | 仓库编码 |

**SpEntry**

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| id | string | 仓位Id |
| name | string | 仓位名称 |
| number | string | 仓位编码 |

**MaterIalEntry**

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| id | string | 商品id |
| name | string | 商品名称 |
| number | string | 商品编码 |

**AuxPropEntry**

| 参数名称 | 参数类型 | 参数说明 |
| --- | --- | --- |
| id | string | 辅助属性id |
| name | string | 辅助属性名称 |
| number | string | 辅助属性编码 |

#### 响应示例

```json
{
  "data": {
    "count": 30,
    "current_page_size": 10,
    "page": 1,
    "page_size": 10,
    "rows": [
      {
        "aux_prop_entry": {
          "id": "758026143428302848",
          "name": "XL",
          "number": "001"
        },
        "aux_qty": "0",
        "batch_no": "1",
        "id": "758026143428302848",
        "kf_date": "2019-11-01",
        "kf_period": 12,
        "kf_type": "2",
        "material_entry": {
          "id": "758026143428302848",
          "name": "758026143428302848",
          "number": "758026143428302848"
        },
        "qty": "2.0000000000",
        "sp_entry": {
          "id": "758026143428302848",
          "name": "001库",
          "number": "001"
        },
        "stock_entry": {
          "id": "758026143428302848",
          "name": "广州仓",
          "number": "001"
        },
        "valid_aux_qty": "0",
        "valid_date": "2020-11-01",
        "valid_qty": "2.0000000000"
      }
    ],
    "total_page": 3
  },
  "description": "success",
  "errcode": 0
}
```

#### 返回码

| 返回码 | 描述 | 解决方案 |
| --- | --- | --- |
| [公告返回码](https://open.jdy.com/#/files/api/detail?index=2&categrayId=1f51c576013945e2af68ef15d4245a48&id=525e704824d24b178ab466530456c037) | 公告返回码 |  |
| 2000002000 | 系统错误,请到开发者社区提单反馈 | [开发者社区](https://vip.kingdee.com/developer?productLineId=29) |
